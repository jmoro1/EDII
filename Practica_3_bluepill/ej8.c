...
void SysTick_handler(void)
{
	...
	GPIO_C->GPIO_ODR ^= -1; // CAMBIO EL ESTADO DEL ODR ENTERO
}

void Systick_delay(uint32_t delay_ms)
{
	uint32_t CalibrationValue_10ms = SysTick->SYST_CALIB; //RELOAD VALUE PARA 10ms
    uint32_t CalibrationValue_1ms = CalibrationValue_10ms /10; //modifico el reload value para tener un tiempo de 1ms
	uint32_t Usr_delay = CalibrationValue_1ms * delay_ms; //seteo el delay deseado en ms
	SysTick->SYST_RVR |= Usr_delay ; //1° SETEO EL RELOAD VALUE 
    SysTick->SYST_CVR |= (0x1 << 0) ; //2° - ESCRIBO UN 1 EN EL PRIMER BIT DEL REGISTRO CURRENT VALUE PARA LIMPIARLO
	SysTick->SYST_CSR |= ((0x1 << CSR_ENA_BIT) | (0x1 << CSR_TICKINT_BIT) | (0x0 << CSR_CLKSRC_BIT) ); //seteo 101, ENABLE,TICKINT Y CLOCK EXTERNO
}	
